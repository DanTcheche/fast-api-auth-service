...
from app.api.dependencies.get_db import SessionDep
...
def get_current_user(session: SessionDep, token: TokenDep) -> UserInDBBase:
    try:
        token_data = validate_token(token)
    except InvalidCredentialsException as e:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=e.message)
    provider = UsersService(session, user_repository).get_by_id(token_data.user_id)
    if not provider:
